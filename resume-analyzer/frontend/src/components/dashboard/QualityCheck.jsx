...
import { Shield, AlertTriangle, CheckCircle2, XCircle, Loader2, FileWarning, X } from 'lucide-react';
                            ...
                            <div className="p-5 bg-gradient-to-r from-emerald-50/60 to-teal-50/40 border border-emerald-100 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-4">
                                <div>
                                    <h3 className="text-sm font-bold text-emerald-950">
                                        Quality Score: {report.quality_score || 88}/100
                                    </h3>
                                    <p className="text-xs text-emerald-800 mt-0.5">
                                        {report.summary || 'Resume demonstrates strong authenticity with realistic metric claims.'}
                                    </p>
                                </div>
                            </div>
                                    ...
                                    (report.issues || []).map((issue, i) => (
                                        <div key={i} className="p-4 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-1.5">
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-xs font-bold text-slate-900">{issue.title || issue.issue}</span>
                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${getSeverityColor(issue.severity)}`}>
                                                    {issue.severity}
                                                </span>
                                            </div>
                                            <p className="text-xs text-slate-600">{issue.description || issue.detail}</p>
                                            {issue.suggestion && (
